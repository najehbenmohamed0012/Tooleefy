...
function getOgImageUrl(param, blogPostId) {
  ...
  try {
    ...
    const hasValidExtension = pathnameLower.endsWith(".jpg") || pathnameLower.endsWith(".jpeg") || pathnameLower.endsWith(".png") || pathnameLower.endsWith(".webp");
...
function runOgImageUrlValidationTests() {
  ...
  const testCases = [
    { param: "home", blogPostId: void 0, expected: "https://tooleefy.com/og/home.jpg" },
    { param: "invoice", blogPostId: void 0, expected: "https://tooleefy.com/og/invoice-generator.jpg" },
    { param: "data:image/png;base64,...", blogPostId: "art-3", expected: "https://tooleefy.com/og/blog/art-3.jpg" },
    { param: "https://images.unsplash.com/photo-1234.jpg", blogPostId: void 0, expected: "https://images.unsplash.com/photo-1234.jpg" },
    { param: "/images/custom-cover.webp", blogPostId: void 0, expected: "https://tooleefy.com/images/custom-cover.webp" }
  ];
  ...
  for (const badUrl of invalidUrlsToTest) {
    const resolved = getOgImageUrl(badUrl, void 0);
...
function buildSeoEditorialHtml(route) {
  try {
    ...
    const jsonPath = import_path.default.join(process.cwd(), "src/utils/seo-editorial-content.json");
    if (!import_fs.default.existsSync(jsonPath)) {
      return "";
    }
    const rawContent = import_fs.default.readFileSync(jsonPath, "utf-8");
    ...
    console.error("Error building SEO Editorial HTML:", error);
    return "";
  }
}
async function startServer() {
  ...
  const PORT = rawPort ? isSocket ? rawPort : Number(rawPort) : 3e3;
  app.disable("x-powered-by");
  app.use((req, res, next) => {
    try {
      const logLine = `[${(/* @__PURE__ */ new Date()).toISOString()}] ${req.method} URL:${req.url} PATH:${req.path} QUERY:${JSON.stringify(req.query)} IP:${req.ip} HOST:${req.headers.host}
`;
      if (!process.env.VERCEL) {
        const logFile = import_path.default.join(process.cwd(), "passenger-debug.log");
        import_fs.default.appendFileSync(logFile, logLine, "utf-8");
      } else {
  ...
  app.use((req, res, next) => {
    ...
    if (req.query._route_) {
      ...
      const queryString = queryKeys.length > 0 ? "?" + queryKeys.map((k) => `${encodeURIComponent(k)}=${encodeURIComponent(queryParams[k])}`).join("&") : "";
  ...
  const ANALYTICS_FILE = import_path.default.join(process.cwd(), "analytics-data.json");
  function loadGlobalAnalytics() {
    try {
      if (import_fs.default.existsSync(ANALYTICS_FILE)) {
        const content = import_fs.default.readFileSync(ANALYTICS_FILE, "utf-8");
  ...
  function saveGlobalAnalytics() {
    try {
      if (!process.env.VERCEL) {
        import_fs.default.writeFileSync(ANALYTICS_FILE, JSON.stringify(globalAnalytics, null, 2), "utf-8");
      } else {
  ...
  app.post("/api/analytics/track", (req, res) => {
    try {
      ...
      if (type === "view") {
        ...
        const rawGeoHeader = req.headers["cf-ipcountry"] || req.headers["x-country-code"] || req.headers["x-appengine-country"] || req.headers["x-visitor-country"];
        if (rawGeoHeader) {
        ...
        if (ip && ip !== "127.0.0.1" && ip !== "::1" && !ip.startsWith("10.") && !ip.startsWith("192.168.") && !ip.startsWith("172.")) {
          if (geoCache.has(ip)) {
            ...
            fetch(`http://ip-api.com/json/${ip}`).then((r) => r.json()).then((resData) => {
  ...
  const serveCrawlFile = (fileName, contentType) => {
    return (req, res) => {
      const baseDir = typeof __dirname !== "undefined" ? import_path.default.join(__dirname, "..") : process.cwd();
      const distDir = typeof __dirname !== "undefined" ? __dirname : import_path.default.join(process.cwd(), "dist");
      const pathsToTry = [
        import_path.default.join(distDir, fileName),
        import_path.default.join(baseDir, "public", fileName),
        import_path.default.join(baseDir, fileName)
      ];
  ...
  const getSupabaseClient = () => {
    ...
    if (supabaseUrl && supabaseAnonKey) {
      try {
        supabaseClient = (0, import_supabase_js.createClient)(supabaseUrl, supabaseAnonKey);
...
`);
  ...
  app.get("/sitemap.xml", async (req, res) => {
    try {
      ...
      if (client) {
        try {
          const { data, error } = await client.from("blog_posts").select("id, date").order("date", { ascending: false });
          if (!error && data && data.length > 0) {
            blogPosts = data.filter((p) => p.id && p.id !== "tooleefy_system_settings_v1");
          }
        } catch (err) {
...
`;
  ...
  app.get("/api/og-image", (req, res) => {
    const tool = req.query.tool || "home";
    const titleMap = {
  ...
  app.get("/api/og-image/:filename", (req, res) => {
    ...
    const imagePath = import_path.default.join(distPath, "images", filename);
  ...
  app.get("/og/blog/:postId.jpg", async (req, res) => {
    ...
    if (client && postId) {
      try {
        const { data, error } = await client.from("blog_posts").select("coverImage").eq("id", postId).maybeSingle();
        if (!error && data) {
    ...
    const fallbackPath = import_path.default.join(distPath, "og", "blog.jpg");
  ...
  async function fetchWithTimeout(url, timeoutMs, defaultData) {
  ...
  app.get("/api/exchange-rates", async (req, res) => {
    ...
    try {
      const fiatData = await fetchWithTimeout("https://api.frankfurter.app/latest?from=USD", 2e3, { rates: DEFAULT_FIAT_RATES });
      ...
      const parsedCrypto = Array.isArray(cryptoData) ? cryptoData.map((c) => ({
        id: c.id,
        symbol: c.symbol,
        name: c.name,
  ...
  function generateFallbackArticle(prompt, category, tone, targetLength, keywords) {
    ...
    const toneDesc = cleanTone.toLowerCase() === "playful" ? "an engaging, energetic, and approachable" : cleanTone.toLowerCase() === "technical" ? "a detailed, analytic, and precise" : "a professional, clear, and action-oriented";
...
`;
  ...
  app.post("/api/ai/write", async (req, res) => {
    try {
      ...
      const keywordsList = Array.isArray(primaryKeywords) ? primaryKeywords.join(", ") : primaryKeywords || "";
...
Target Length: ~${targetLength || 1e3} words
Primary SEO Keywords to include: "${keywordsList}"`;
        ...
        for (const modelName of modelsToTry) {
          ...
          for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
              ...
              const isTransient = errStatus === 503 || errMsg.includes("503") || errMsg.includes("UNAVAILABLE") || errMsg.includes("Unexpected token '<'") || errMsg.includes("not valid JSON");
  ...
  async function runModelWithFallback(client, prompt, systemInstruction, responseSchema) {
    const modelsToTry = ["gemini-3.5-flash", "gemini-flash-latest", "gemini-3.1-flash-lite"];
    let lastError = null;
    let result = null;
    for (const modelName of modelsToTry) {
  ...
  app.post("/api/ai/improve", async (req, res) => {
    try {
      ...
      const actionStr = Array.isArray(action) ? action.map((act) => act.toUpperCase()).join(", ") : (action || "Analyze Article").toUpperCase();
      if (mode === "analyze") {
        ...
        const responseSchema = {
          ...
          properties: {
            analysis: {
              type: import_genai.Type.OBJECT,
              properties: {
                content_quality: { type: import_genai.Type.STRING },
                heading_structure: { type: import_genai.Type.STRING },
                readability: { type: import_genai.Type.STRING },
                seo: { type: import_genai.Type.STRING },
                spacing: { type: import_genai.Type.STRING },
...
${article.content}`;
        const responseSchema = {
          ...
          properties: {
            ...
            seoDescription: { type: import_genai.Type.STRING },
            seoKeywords: { type: import_genai.Type.STRING }
          },
    ...
    if (!isProfilePage && (reqPath === "/" || reqPath === "/index.html" || reqPath.startsWith("/blog"))) {
      ...
      if (client) {
        try {
          ...
          if (!error && data) {
            allPosts = data.filter((p) => p.id !== "tooleefy_system_settings_v1");
          }
        } catch (err) {
...
`;
        ...
        if (postId) {
          ...
          if (client) {
            try {
              const { data, error } = await client.from("blog_posts").select("*").eq("id", postId).maybeSingle();
              if (!error && data) {
                foundPost = data;
              }
            } catch (err) {
...
${seoTags}
    ...
    const seoHtml = buildSeoEditorialHtml(reqPath);
    if (seoHtml) {
      html = html.replace('<div id="root"></div>', `<div id="root">${seoHtml}</div>`);
    }
    return html;
  }
  const distPath = typeof __dirname !== "undefined" && (__dirname.endsWith("dist") || __dirname.includes("/dist/") || __dirname.includes("\\dist")) ? __dirname : import_path.default.join(process.cwd(), "dist");
  const isRunningBundle = typeof __filename !== "undefined" && (__filename.endsWith(".cjs") || __filename.includes("dist")) || typeof __dirname !== "undefined" && (__dirname.endsWith("dist") || __dirname.includes("/dist/") || __dirname.includes("\\dist"));
  ...
  if (!isProduction) {
    ...
    app.use(import_express.default.static(distPath, {
      maxAge: "1d",
      index: false,
      // Prevents default index.html from overriding the dyn seo star controller
      redirect: false,
      // Prevents express.static from redirecting directory paths to trailing slashes (fixes Vercel redirect loops)
    ...
    app.get("*", async (req, res) => {
      try {
        ...
        if (cleanPath.startsWith("/blog/") && cleanPath !== "/blog") {
          ...
          if (client && postId) {
            try {
              const { data, error } = await client.from("blog_posts").select("id").eq("id", postId).maybeSingle();
              if (!error && data) {
                postExists = true;
              }
            } catch (err) {
        ...
        if (isStaticAsset) {
          ...
          const physicalPath = import_path.default.join(distPath, assetSubPath);
          if (import_fs.default.existsSync(physicalPath) && import_fs.default.statSync(physicalPath).isFile()) {
            const ext = import_path.default.extname(physicalPath).toLowerCase();
        ...
        const isDynamicPage = cleanPath === "/" || cleanPath === "/index.html" || cleanPath.startsWith("/tools/") || cleanPath === "/blog" || cleanPath.startsWith("/blog/") || cleanPath === "/categories" || cleanPath === "/about" || cleanPath === "/faq" || cleanPath === "/contact" || cleanPath === "/value-our-tools" || cleanPath === "/privacy" || cleanPath === "/terms" || cleanPath === "/cookies";
        if (!isDynamicPage && cleanPath !== "/" && cleanPath !== "/index.html" && !cleanPath.startsWith("/api/")) {
          const staticFileCandidates = [
            import_path.default.join(distPath, cleanPath, "index.html"),
            import_path.default.join(distPath, cleanPath + ".html")
          ];
          let matchedStaticFile = "";
          for (const candidate of staticFileCandidates) {
            if (import_fs.default.existsSync(candidate) && import_fs.default.statSync(candidate).isFile()) {
        ...
        if (import_fs.default.existsSync(indexPath)) {
          if (!indexHtmlCache) {
            const rawHtml = import_fs.default.readFileSync(indexPath, "utf-8");
